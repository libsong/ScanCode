...
#define DEVID 0 //标识生产的第 n 台设备 生产时考虑其他方式获取，如硬件存储
...
extern volatile unsigned long long sysIntCnt;
...
void heartBeat (void)
{
	...
	if (sysIntCnt % HEARTBEAT_CYCLE == 0) {
		beat[0] = 0x00;
		beat[1] = 0x00;
		beat[2] = 0x00;
		beat[3] = 0x00;
		...
		USART2_Send((char *)beatP, 10);		//注意是否会与扫码上传混合
	}	
